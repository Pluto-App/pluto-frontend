...
const InitWindowShareCanvas = React.memo((props) => {
        ...
        (uid) => {
          ...
          localStream.init(
            async () => {
              ...
              var overlayBounds = await ipcRenderer.sendSync(
                'windowshare-source-bounds',
                sourceInfo
              );
  ...
  useEffect(() => {
    ...
    AgoraRTC.getScreenSources(function (err, sources) {
      var windowSources = sources.filter(function (source) {
  ...
  return (
    <div>
      <div className="mb-4 text-lg px-3 center">Select Window to share..</div>
