...
const windowshare_resolutions = JSON.parse(
  localStorage.getItem('windowshare_resolutions')
);
...
const StreamWindowShareCanvas = React.memo((props) => {
  ...
  const [windowShareResolution, setWindowShareResolution] = useState(
    windowshare_resolutions[currentWindow.data.user_uid]
  );
  ...
  const removeStream = (uid) => {
    ...
    //   userColor: null,
    // });
    if (streamList.length == 0) exitWindowShare();
  };
  ...
  const shareCursorData = async (e) => {
    try {
      ...
      var eventData = {
        ...
        keyCode:
          e.keyCode || (e.nativeEvent ? e.nativeEvent.keyCode : undefined),
  ...
  function Area(Increment, Count, Width, Height, Margin = 0) {
    ...
    var ratio = windowShareResolution
  ...
  useEffect(() => {
    ...
    AgoraClient.init(props.config.appId, async () => {
      ...
      AgoraClient.join(
        ...
        props.config.user_uid + '-' + Date.now(),
        (uid) => {
    ...
    socket_live.on(events.windowShareCursor, (data) => {
      if (data.user.id && !windowShareViewersRef.current[data.user.id]) {
        setWindowShareViewers({
          ...windowShareViewersRef.current,
  ...
  return (
    <div id="ScreenShareDish">
      <div id="controls-topbar" style={controlsTopBarStyle}></div>
      <div style={streamContainerStyle}>
        ...
        >
          <div
            id="ag-screen"
            className="ScreenShareCamera"
            tabIndex="0"
            style={{
              border: '5px solid ' + props.config.owner_color,
              outline: 'none',
              position: 'relative',
            }}
          >
            {Object.keys(windowShareViewers).map((user_id) => (
              <WindowShareCursor
                ...
                streaming={1}
                screenDivBounds={screenDivBounds}
              ></WindowShareCursor>
            ))}
          </div>
          ...
          >
            <div style={{ display: 'table', height: '30px' }}>
              ...
              >
                {props.config.owner.name
                  ? props.config.owner.name.split(' ')[0] + "'s Screen"
                  : ''}
              </span>
            </div>
          </div>
