...
export const Settings = React.memo(() => {
  ...
  const [userPreference, setUserPreference] = useState(
    JSON.parse(localStorage.getItem('userPreference')) || state.userPreference
  );
  ...
  const createTeam = async (e) => {
    ...
    if (newTeam.name.length > 2) {
      ...
      var team = await actions.team.createTeam({
        authData: authData,
        teamData: teamData,
      });
      ...
      if (currentUser) {
        ...
        setActiveTeam(
          await actions.team.getTeam({ authData: authData, team_id: team.id })
        );
        setActiveTeamUsers(
          await actions.user.getTeamMembers({
            authData: authData,
            teamId: team.id,
          })
        );
  ...
  const updateTeam = async () => {
    if (activeTeam) {
      ...
      var updatedTeam = await actions.team.updateTeam({
        authData: authData,
        teamData: teamData,
      });
  ...
  const updateUserPreference = async () => {
    ...
    if (userPreference.id) {
      await actions.userpreference.updateUserPreference({
        authData: authData,
        userPreferenceData: userPreferenceData,
      });
    } else {
      await actions.userpreference.createUserPreference({
        authData: authData,
        userPreferenceData: userPreferenceData,
      });
  ...
  useEffect(() => {
    async function fetchUser() {
      setCurrentUser(
        await actions.user.getLoggedInUser({ authData: authData })
      );
    }
    fetchUser();
  }, [actions, authData]);
  ...
  return (
    <div className="w-full flex" style={containerStyle}>
      <div
        ...
        style={{ width: '200px', background: '#202225' }}
      >
        ...
        <div>
          <div className="pt-5">
            ...
            >
              <div className="flex items-center justify-center overflow-hidden">
                <i
                  className="material-icons md-light md-inactive mr-3"
                  style={{ fontSize: '24px' }}
                >
                  tune
                </i>
          ...
          <div
            className="mt-8"
            style={{ height: '1px', width: '100%', background: '#484e52' }}
          ></div>
          ...
          >
            ...
            >
              <div
                className="mr-3 bg-black flex items-center justify-center overflow-hidden"
                style={{ width: '34px', height: '34px', float: 'left' }}
              >
                <i
                  className="material-icons hover:bg-gray-700"
      ...
      <div className="flex-1 text-white">
        <div className="flex">
          <div
            className="w-full draggable-elem w-full"
            style={{ height: '20px' }}
          ></div>
          {isMac ? (
            ''
        ...
        <div
          className="flex-1 px-12 text-white pt-2"
        ...
        >
          {activeSection === 'profile' && (
            <div>
              ...
              <div className="flex">
                ...
                <div className="ml-3">
                  <input
                    className="shadow appearance-none border rounded w-full py-1 px-5 text-gray-700"
                    style={{ width: '100%' }}
                    onChange={(e) =>
          ...
          {activeSection === 'preferences' && (
            <div>
              ...
              <p className="text-grey text-md tracking-wide mt-12 mb-8">
                Active App Sharing Settings
              </p>

              <div className="flex">
                <div className="flex items-center justify-center text-grey text-sm mb-1 overflow-hidden">
                  <p className="text-white">
                    Show Active App to Team Members:{' '}
                  </p>
                </div>
                <div className="ml-3">
                  <input
                    className="shadow border py-1 px-5 text-gray-700"
                    type="checkbox"
              ...
              <div className="flex mt-3">
                <div className="flex items-center justify-center text-grey text-sm mb-1 overflow-hidden">
                  <p className="text-white">
                    Allow Users To Join On Active App:{' '}
                  </p>
                </div>
                <div className="ml-3">
                  <input
              ...
              <p className="text-grey text-md tracking-wide mt-12 mb-8">
                Supported Active Apps
              </p>

              <div className="" style={{ maxHeight: 'calc(100vh - 400px)' }}>
                {supportedAppsList &&
                  Object.keys(supportedAppsList).map((key) => (
                    <li
                    ...
                    >
                      ...
                      <div>{supportedAppsList[key].name}</div>
                    </li>
                  ))}
              </div>
            </div>
          ...
          {activeSection === 'team' && (
            <div>
              ...
              {confirmTeamDelete && (
                <div
                  className="items-center absolute rounded-lg mx-1 p-1 py-1"
                  style={confirmDialogueStyle}
                >
