...
export default function HomePage() {
  ...
  const rateApp = async (e) => {
    ...
    await actions.rating.createRating({
      authData: authData,
  ...
  useEffect(() => {
    if (state.noTeams) history.push('/add-team');
  ...
  return (
    <div className="w-full flex main-container">
      <div
        className="w-full ml-15 flex-1 text-white"
      ...
      >
        ...
        <div className="rooms-list-container" style={{ height: 'relative' }}>
          <div
            className="flex justify-between items-center p-1 pl-1"
          ...
          >
            ...
            <button
              className="text-white focus:outline-none btn-add"
              onClick={(e) => {
                e.stopPropagation();
                setAddingRoom(true);
              }}
          ...
          {addingRoom && (
            <div className="flex justify-center items-center hover:bg-gray-800">
              <input
                className="shadow appearance-none border rounded w-full py-1 
          ...
          {state.teamRooms.length > 4 && (
            <div className="center">
              <i className="material-icons md-light md-inactive mr-2">
                expand_more
              </i>
            </div>
          )}
        ...
        <div className="members-list-container" style={{ height: 'relative' }}>
          <div className="flex justify-between items-center p-1 pl-1">
            <div
              className="px-3 font-bold tracking-wide"
              style={{ fontWeight: '600', color: '#BABBBE', fontSize: '14px' }}
            >
              TEAM MATES
            </div>
          </div>
...

        {
          ...
          showInviteModal && state.currentTeam.id ? (
            <div
              className="items-center absolute rounded-sm bg-white mx-2 p-1 py-1"
              style={inviteModalStyle}
              onClick={(e) => {}}
            >
              <div className="flex w-full">
                <h4 className="font-bold text-xl text-gray-600 text-center mb-2 flex-1">
                  {' '}
                  Add Teammates
                </h4>
                <h4
              ...
              >
                {!copySuccess ? 'Copy Invite' : 'Copied !!'}
              </button>
            </div>
          ) : (
            <div></div>
