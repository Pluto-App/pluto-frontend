...
const RoomListItem = React.memo((props) => {
  ...
  const startVideo = async () => {
    const callChannelId = localStorage.getItem('call_channel_id');
    if (callChannelId)
      await actions.app.clearVideoCallData({
        call_channel_id: callChannelId,
      });
    ...
    actions.app.setUserInRoom({
      room_id: room.id,
      user_uid: state.userProfileData.uid,
    });

    ipcRenderer.send('set-call-data', { call_data: call_data });
    ipcRenderer.send('init-video-call-window', {
      call_channel_id: call_channel_id,
      call_data: call_data,
    });
  ...
  return (
    ...
    >
      {editingRoom ? (
        ...
        >
          <input
            className="shadow appearance-none border rounded w-full py-1 px-5 text-gray-700 leading-tight focus:outline-none 
            ...
            onKeyUp={(e) => {
              if (e.keyCode === 13 || e.which === 13) {
                e.target.value === ''
                  ? ToastNotification('error', 'Room Name Empty !')
                  : setEditingRoom(false);
        ...
        <div
          className="flex py-0 justify-between p-1"
          id={room.id}
          style={{
            background:
              !state.teamRoomsMap[room.id] ||
              !(state.teamRoomsMap[room.id].users || []).includes(
                state.userProfileData.uid
              )
                ? ''
                : '#202225',
          }}
        >
          <div className="flex justify-start p-2" style={{ width: '100%' }}>
            {Object.keys((state.teamRoomsMap[room.id] || {}).users || [])
              .length > 0 && activeAppInfo.logo ? (
              <div
                className="flex"
                style={{
                  width: '30px',
            ...
            <div
              className="text-white px-2 font-bold tracking-wide text-xs pointer"
              style={{
                whiteSpace: 'nowrap',
                fontSize: '14px',
                minHeight: '30px',
          ...
          {hoverState && (
            <div className="flex" style={{ padding: '5px' }}>
              {(!state.teamMembersMap[state.userProfileData.id] ||
                !state.teamMembersMap[state.userProfileData.id].in_call) && (
                <button
                  className="text-white focus:outline-none bg-light-blue btn-join-room"
              ...
              <button
                className="text-gray-300 hover:text-indigo-500 px-1 focus:outline-none"
                onClick={() => {
      ...
      {((state.teamRoomsMap[room.id] || {}).users || []).length > 0 && (
        <div className="flex px-3 p-2" style={{}}>
          {((state.teamRoomsMap[room.id] || {}).users || []).map(
            (uid, index) => (
              <div
                key={index}
